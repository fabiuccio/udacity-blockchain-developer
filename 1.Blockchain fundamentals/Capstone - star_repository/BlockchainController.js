...
class BlockchainController {
    ...
    // Endpoint that allows user to validate the chain
    requestChainValidation() {
        this.app.get("/validateChain", async(req, res) => {

            //Checking the chain validation
            let errorLog = await this.blockchain.validateChain();

            //If no errors are returned the chain is valid
            if(errorLog.length===0) {
                return res.status(200).send("The blockchain is valid!");
                
            } 
            else {
                return res.status(500).send("There's an issue in the blockchain, please find the error messages: " + errorLog);
            }
        });
    }
